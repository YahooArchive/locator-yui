        ...
        for (key in source) {
            if (source.hasOwnProperty(key)) {
                if (obj.hasOwnProperty(key) && obj[key] !== source[key]) {
                    console.warn("Naming collision: the same name has been used already:" + key);
                }
                obj[key] = source[key];
            }
        }
...
@param {Object} list original `evt.files` from locator plugin
@param {Function} filter Custom function to analyze each file
    @param {Object} filter.bundle 
    @param {String} filter.relativePath
@return {Array} the list of absolute paths files to be shifted or empty array
**/
exports.filterFilesInBundle = function (bundle, list, filter) {
    ...
    Object.keys(list || {}).forEach(function (element) {
        // filtering out files based on folder if neded
