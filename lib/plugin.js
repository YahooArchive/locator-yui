...
PluginClass.prototype = {
    ...
    **/
    ...
    /**
    Creates a function that generates the loader metadata for the `server` runtime.
    ...
    @return {Function}
    **/
    generateServerDataFactory: function (bundle) {
        var self = this,
            bundleName = bundle.name;
        return function generateServerData(fulfilled) {
            // dealing with server stuff
            var loaderData = self.getLoaderData(bundleName, function (name, config) {
    ...
    /**
    Creates a function that generates the loader metadata for the `client` runtime.
    ...
    @return {Function}
    **/
    generateClientDataFactory: function (bundle, moduleName) {
        var self = this,
            bundleName = bundle.name;
        return function generateClientData(fulfilled) {
    ...
    @protected
    @param {Object} bundle
    @param {String} cssproc optional
    @param {Object} builds
    @param {Object} shifter
    **/
    shiftEverythingFactory: function (bundle, cssproc, builds, shifter) {
        var self = this;

        return function shiftEverything(fulfilled, rejected) {
            var args = [].concat(self.describe.args);
    ...
    /**
    Creates a function that attaches the `client` loader metadata to 
    the bundle.
    ...
    @param {String} destination_path fs path where to write the loader metadata file
    @return {Function}
    **/
    attachClientDataFactory: function (bundle, api, destination_path) {
        var bundleName = bundle.name;
        return function (loaderData) {
            if (loaderData) {
            ...
            return loaderData && api.writeFileInBundle(bundleName, destination_path, loaderData.js);
        };
    },

    /**
    Creates a function that attaches the `server` loader metadata to 
    the bundle.
    ...
    @return {Function}
    **/
    attachServerDataFactory: function (bundle) {
        return function (loaderData) {
            if (loaderData) {
    ...
    bundleUpdated: function (evt, api) {
        ...
        generateServerData = self.generateServerDataFactory(bundle);
        generateClientData = self.generateClientDataFactory(bundle, moduleName);
        shiftEverything = self.shiftEverythingFactory(bundle, cssproc, builds, shifter);
        attachServerData = self.attachServerDataFactory(bundle);
        attachClientData = self.attachClientDataFactory(bundle, api, destination_path);

        return api.promise(generateServerData)
            .then(attachServerData)
            .then(function () {
                return api.promise(generateClientData);
            })
            .then(attachClientData)
